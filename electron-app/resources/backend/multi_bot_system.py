...
class TradeBot(BaseBot):
    ...
    async def _execute_signal(self, signal: Dict, settings: dict) -> bool:
        ...
        # UND: Simuliere Risiko nach neuem Trade, passe Lot ggf. an
        # ═══════════════════════════════════════════════════════════════════
        try:
            ...
            # Lot-Berechnung vorbereiten (wie im System üblich)
            # Hole relevante Parameter
            confidence = signal.get('confidence', 0)
            stop_loss_pips = signal.get('stop_loss_pips', 20)  # Default 20
            tick_value = signal.get('tick_value', 10.0)  # Default 10
            trading_mode = signal.get('strategy', 'neutral')
            # Hole Lot-Berechnungsmethode
            lot_size = self._calculate_lot_size_v2(
                balance=total_equity if total_equity > 0 else 0,
            ...
            # Simuliere Risiko nach neuem Trade
            # Annahme: Risiko = Lot * StopLossPips * TickValue
            new_trade_risk = lot_size * stop_loss_pips * tick_value
            new_total_risk = abs(total_unrealized_pnl) + new_trade_risk
            if total_equity > 0:
                new_risk_percent = new_total_risk / total_equity * 100
                if new_risk_percent > max_portfolio_risk:
                    # Versuche Lot zu reduzieren, sodass Risiko-Limit eingehalten wird
                    max_allowed_risk = max_portfolio_risk / 100 * total_equity - abs(total_unrealized_pnl)
                    if max_allowed_risk <= 0:
                        logger.warning(f"🚫 PORTFOLIO-RISIKO ZU HOCH: {new_risk_percent:.1f}% > {max_portfolio_risk}% (kein Spielraum)")
                        ...
                        return False
                    # Berechne maximal erlaubten Lot
                    max_lot = max_allowed_risk / (stop_loss_pips * tick_value)
                    if max_lot < 0.01:
                        logger.warning(f"🚫 PORTFOLIO-RISIKO: Erlaubter Lot {max_lot:.3f} < 0.01 - Trade blockiert!")
                        return False
                    logger.warning(f"⚠️ Lot {lot_size:.2f} würde Risiko-Limit überschreiten. Reduziere auf {max_lot:.2f}")
                    lot_size = round(max_lot, 2)
                    # Optional: Setze lot_size im Signal für spätere Orderausführung
                    signal['lot_size'] = lot_size
                else:
                    logger.debug(f"✅ Portfolio-Risiko nach neuem Trade OK: {new_risk_percent:.1f}% <= {max_portfolio_risk}%")
                    signal['lot_size'] = lot_size
        except Exception as e:
        ...
        for platform in active_platforms:
            ...
            try:
                ...
                for pos in positions:
                    ...
                    # Peak-Profit-Tracking & Persistenz für echte Trades
                    try:
                        from autonomous_trading_intelligence import autonomous_trading
                        profit = pos.get('profit', 0)
                        trade_id = f"mt5_{ticket}"
                        # Registriere Trade für Risk-Engine, falls noch nicht vorhanden
                        ...
                        # Risk-Engine aufrufen (tracked peak_profit und persistiert)
                        ...
                        logger.info(
                            f"[PEAK-MONITOR] {pos.get('symbol','?')}#{ticket} profit={profit:.2f}, peak_profit={peak_profit}, peak_progress={peak_progress}, elapsed_min={elapsed}"
                        )
                        # Automatisches Schließen bei Profit-Drawdown-Exit
                        if risk_action and risk_action.get('action') == 'profit_drawdown_exit':
                            logger.info(f"🚨 PROFIT-DRAWDOWN-EXIT: {pos.get('symbol','?')}#{ticket} wird automatisch geschlossen! Grund: {risk_action.get('reason')}")
                            close_result = await multi_platform.close_position(platform, str(ticket))
                            if close_result:
                                closed_count += 1
                                logger.info(f"✅ Position {ticket} closed: PROFIT-DRAWDOWN-EXIT @ {current_price:.2f}")
                                continue  # Nach dem Schließen nächste Position prüfen
                    except Exception as e:
                        logger.debug(f"Peak-Profit-Tracking Fehler für {ticket}: {e}")
