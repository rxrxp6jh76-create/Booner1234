            ...
            for doc in market_docs:
                ...
                if commodity_id:
                    ...
                    try:
                        ...
                        cursor = await self.db.market_data_history.find({"commodity": commodity_id})
                        history_cursor = cursor.sort("timestamp", -1).limit(250)
...
"""
    ...
    async def get_all_open_ai_positions(self) -> List:
        ...
        try:
            cursor = await self.db.trade_settings.find({
                "status": {"$in": ["OPEN", "ACTIVE"]},
                "strategy": {"$in": ["swing", "day", "scalping", "mean_reversion", "momentum", "breakout", "grid"]}
            })
            positions = await cursor.to_list(1000)
            return positions
