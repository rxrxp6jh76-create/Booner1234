...
class Database:
    ...
    async def migrate_peak_profit(self):
        # Peak-Profit für offene Trades in trade_settings
        try:
            await self._conn.execute("ALTER TABLE trade_settings ADD COLUMN peak_profit REAL")
            logger.info("✅ Added peak_profit column to trade_settings table")
        except Exception:
    ...
    async def initialize_schema(self):
        ...
        try:
            ...
            # V3.3.x: Add peak columns to trades if missing
            try:
                await self._conn.execute("ALTER TABLE trades ADD COLUMN peak_high REAL")
                logger.info("✅ Added peak_high column to trades table")
            except:
                pass  # Column already exists
            try:
                await self._conn.execute("ALTER TABLE trades ADD COLUMN peak_low REAL")
                logger.info("✅ Added peak_low column to trades table")
            except:
                pass  # Column already exists
            try:
                await self._conn.execute("ALTER TABLE trades ADD COLUMN peak_entry REAL")
                logger.info("✅ Added peak_entry column to trades table")
            except:
                pass  # Column already exists
